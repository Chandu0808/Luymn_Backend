...
from sqlalchemy import Date, func, case, distinct, cast, String, and_, or_, text
...
def get_space_utilization_by_area(
    ...
    for area in areas:
        ...
        # Display name: "floor.name / area.name" when floor exists, else area.name
        area_name = (area.name or "").strip() or ""
        if area.floor and getattr(area.floor, "name", None):
            floor_name = (area.floor.name or "").strip()
            display_name = f"{floor_name} / {area_name}" if floor_name else area_name
        else:
...
def _strategy_display_name(internal_key: str) -> str:
    """Map internal strategy key to API response display name. Internal logic unchanged."""
    if internal_key == "Keypad":
        return "Personal Control"
    return internal_key


def _strategy_data_for_response(data: Dict[str, Any]) -> Dict[str, Any]:
    """Build response data dict with display name for Keypad only. Internal keys unchanged elsewhere."""
...
def get_unified_energy_data_of_a_day(db: Session, area_ids, floor_ids, data_date):
    ...
    if area_ids and len(area_ids) <= 4:
        return get_data_of_individual_areas_of_a_day(db, x_axis, area_ids, data_date)
    ...
    return {
        ...
        "consumption_data": {
            "combined_areas": consumption,
        },
        "savings": savings, 
        "savings_data": {
            "combined_areas": savings,
        },
        "max_limit": limit, 
        "unit": unit, 
...
def get_data_of_individual_areas_of_a_day(db: Session, x_axis, area_ids, data_date):
    consumption_data = {}
    consumption_data_combined = []
    filling_consumption_data_combined_first_time = True
    savings_data = {}
    savings_data_combined = []
    filling_savings_data_combined_first_time = True
    peak_consumption = 0
    min_consumption = 0
    peak_savings = 0
    peak_consumption_time = None
    ...
    for area_id in area_ids:
        ...
        if area:
            ...
            rows = (
                ...
                .filter(AreaEnergyStat.created_date == data_date)
                .filter(AreaEnergyStat.area_id == area_id)
                .group_by(AreaEnergyStat.timespan_15min)
                .all()
            )
            ...
            if rows:
                for row in rows:
                    time_series[row.timespan_15min] = {
                        "energy_consumed_in_Wh": row.energy_consumed_in_Wh if row.energy_consumed_in_Wh is not None else 'null',
                        "energy_saved_in_Wh": row.energy_saved_in_Wh if row.energy_saved_in_Wh is not None else 'null'
                    }
                    if row.energy_consumed_in_Wh:
                        if peak_consumption_time:
                            if peak_consumption < row.energy_consumed_in_Wh:
                                peak_consumption = row.energy_consumed_in_Wh
                                peak_consumption_time = row.timespan_15min
                        else:
                            peak_consumption = row.energy_consumed_in_Wh
                            peak_consumption_time = row.timespan_15min
                        ...
                        if min_consumption_time:
                            if min_consumption > row.energy_consumed_in_Wh:
                                min_consumption = row.energy_consumed_in_Wh
                                min_consumption_time = row.timespan_15min
                                peak_savings = row.energy_saved_in_Wh
                        else:
                            min_consumption = row.energy_consumed_in_Wh
                            min_consumption_time = row.timespan_15min
                ...
                if rows:
                    ...
                    if row.energy_consumed_in_Wh:
                        if row.energy_consumed_in_Wh > peak_consumption:
                            peak_consumption = row.energy_consumed_in_Wh
                    if row.energy_saved_in_Wh:
                        if row.energy_saved_in_Wh > peak_savings:
                            peak_savings = row.energy_saved_in_Wh
                else:
                    time_series['2400'] = {
                        "energy_consumed_in_Wh": 'null',
                        "energy_saved_in_Wh": 'null'
                    }
                
                consumption_data[area_name] = []
                savings_data[area_name] = []
                for i in range(len(x_axis)):
                    time_span_str = x_axis[i].replace(":", "")
                    if time_span_str in time_series:
                        consumption_data[area_name].append(time_series[time_span_str]["energy_consumed_in_Wh"])
                        savings_data[area_name].append(time_series[time_span_str]["energy_saved_in_Wh"])
                        if filling_consumption_data_combined_first_time:
                            consumption_data_combined.append(time_series[time_span_str]["energy_consumed_in_Wh"])
                        else:
                            if consumption_data_combined[i] != 'null' and time_series[time_span_str]["energy_consumed_in_Wh"] != 'null':
                                consumption_data_combined[i] = consumption_data_combined[i] + time_series[time_span_str]["energy_consumed_in_Wh"]
                            elif time_series[time_span_str]["energy_consumed_in_Wh"] != 'null':
                                consumption_data_combined[i] = time_series[time_span_str]["energy_consumed_in_Wh"]

                        if filling_savings_data_combined_first_time:
                            savings_data_combined.append(time_series[time_span_str]["energy_saved_in_Wh"])
                        else:
                            if savings_data_combined[i] != 'null' and time_series[time_span_str]["energy_saved_in_Wh"] != 'null':
                                savings_data_combined[i] = savings_data_combined[i] + time_series[time_span_str]["energy_saved_in_Wh"]
                            elif time_series[time_span_str]["energy_saved_in_Wh"] != 'null':
                                savings_data_combined[i] = time_series[time_span_str]["energy_saved_in_Wh"]
                    else:
                        consumption_data[area_name].append('null')
                        savings_data[area_name].append('null')
                        if filling_consumption_data_combined_first_time:
                            consumption_data_combined.append('null')
                        if filling_savings_data_combined_first_time:
                            savings_data_combined.append('null')

                if filling_consumption_data_combined_first_time:
                    filling_consumption_data_combined_first_time = False
                if filling_savings_data_combined_first_time:
                    filling_savings_data_combined_first_time = False
    ...
    if unit != "Wh":
        for area_name in consumption_data.keys():
            if unit == "kWh":
                consumption_data[area_name] = [round(value / 1000, 2) if value != 'null' else 'null' for value in consumption_data[area_name]]
                savings_data[area_name] = [round(value / 1000, 2) if value != 'null' else 'null' for value in savings_data[area_name]]
            elif unit == "MWh":
                consumption_data[area_name] = [round(value / 1000000, 2) if value != 'null' else 'null' for value in consumption_data[area_name]]
                savings_data[area_name] = [round(value / 1000000, 2) if value != 'null' else 'null' for value in savings_data[area_name]]

        if unit == "kWh":
            consumption_data_combined = [round(value / 1000, 2) if value != 'null' else 'null' for value in consumption_data_combined]
            savings_data_combined = [round(value / 1000, 2) if value != 'null' else 'null' for value in savings_data_combined]
        elif unit == "MWh":
            consumption_data_combined = [round(value / 1000000, 2) if value != 'null' else 'null' for value in consumption_data_combined]
            savings_data_combined = [round(value / 1000000, 2) if value != 'null' else 'null' for value in savings_data_combined]
    return {
        "chart-type": "day",
        "x-axis": x_axis, 
        "consumption": consumption_data_combined, 
        "consumption_data": consumption_data,
        "savings_data": savings_data,
        "savings": savings_data_combined,
        "max_limit": limit, 
        "unit": unit, 
        "consumption_peak":{"value": peak_consumption, "time": peak_consumption_time}, 
        "consumption_min":{"value": min_consumption, "time": min_consumption_time}
...
def get_data_of_6_hrs(db: Session, area_ids, floor_ids, data_date, data_time_span_6_hrs):
    ...
    return {"consumption": 'null', "savings": 'null'}# This would happen when we query for the date and time where there were no data
...
def get_unified_energy_data_of_a_week(db: Session, area_ids, floor_ids, time_range, start_date, end_date):
    ...
    if area_ids and len(area_ids) <= 4:
        return get_data_of_individual_areas_of_a_week(db, x_axis, area_ids, start_date, end_date)
    ...
    for timespan in x_axis:
        if (
            ...
            if data_of_6_hrs["consumption"] != 'null':
                if peak_consumption_time:
...
def get_data_of_individual_areas_of_a_week(db, x_axis, area_ids, start_date, end_date):
    consumption_data = {}
    consumption_data_combined = []
    filling_consumption_data_combined_first_time = True
    savings_data = {}
        ...
        if area:
            ...
            for timespan in x_axis:
                if (
                    ...
                    data_of_6_hrs = get_data_of_6_hrs(db, [area_id], [], data_date, data_time_span_6_hrs)
                    consumption_data[area_name].append(data_of_6_hrs["consumption"])
                    savings_data[area_name].append(data_of_6_hrs["savings"])

                    if filling_consumption_data_combined_first_time:
                        consumption_data_combined.append(data_of_6_hrs["consumption"])
                    else:
                        if consumption_data_combined[i] != 'null' and data_of_6_hrs["consumption"] != 'null':
                            consumption_data_combined[i] += data_of_6_hrs["consumption"]
                        elif data_of_6_hrs["consumption"] != 'null':
                            consumption_data_combined[i] = data_of_6_hrs["consumption"] 
                    
                    if filling_savings_data_combined_first_time:
                        savings_data_combined.append(data_of_6_hrs["savings"])
                    else:
                        if savings_data_combined[i] != 'null' and data_of_6_hrs["savings"] != 'null':
                            savings_data_combined[i] += data_of_6_hrs["savings"]
                        elif data_of_6_hrs["savings"] != 'null':
                            savings_data_combined[i] = data_of_6_hrs["savings"]
            ...
            if filling_savings_data_combined_first_time:
                filling_savings_data_combined_first_time = False
            i += 1 # index of the for loop for updating combined consumption and savings arrays
...
def get_unified_energy_data_of_a_month(db, area_ids, floor_ids, time_range, start_date, end_date):
    ...
    if area_ids and len(area_ids) <= 4:
        return get_data_of_individual_areas_of_a_month(db, x_axis, time_range, area_ids, start_date, end_date)
...
def get_data_of_individual_areas_of_a_month(db, x_axis, time_range, area_ids, start_date, end_date):
    consumption_data = {}
    consumption_data_combined = []
    filling_consumption_data_combined_first_time = True
    savings_data = {}
    ...
    for area_id in area_ids:
        ...
        if area:
            ...
            if rows:
                ...
                for timespan in x_axis:
                    if timespan in time_series:
                        consumption_data[area_name].append(time_series[timespan]["energy_consumed_in_Wh"])
                        savings_data[area_name].append(time_series[timespan]["energy_saved_in_Wh"])
                        if filling_consumption_data_combined_first_time:
                            consumption_data_combined.append(time_series[timespan]["energy_consumed_in_Wh"])
                        else:
                            if consumption_data_combined[i] != 'null' and time_series[timespan]["energy_consumed_in_Wh"] != 'null':
                                consumption_data_combined[i] += time_series[timespan]["energy_consumed_in_Wh"]
                            elif time_series[timespan]["energy_consumed_in_Wh"] != 'null':
                                consumption_data_combined[i] = time_series[timespan]["energy_consumed_in_Wh"]
                        
                        if filling_savings_data_combined_first_time:
                            savings_data_combined.append(time_series[timespan]["energy_saved_in_Wh"])
                        else:
                            if savings_data_combined[i] != 'null' and time_series[timespan]["energy_saved_in_Wh"] != 'null':
                                savings_data_combined[i] += time_series[timespan]["energy_saved_in_Wh"]
                            elif time_series[timespan]["energy_saved_in_Wh"] != 'null':
                                savings_data_combined[i] = time_series[timespan]["energy_saved_in_Wh"]
...
def get_unified_energy_data_of_a_year(db, area_ids, floor_ids, time_range, start_date, end_date):
    ...
    if(time_range == "this_year"):
        ...
        end_date = end_date.replace(day=8) if end_span == 1 else end_date.replace(day=15) if end_span == 2 else end_date.replace(day=22) if end_span == 3 else end_date.replace(day=calendar.monthrange(end_date.year, end_date.month)[1])
    ...
    if area_ids and len(area_ids) <= 4:
        return get_data_of_individual_areas_of_a_year(db, area_ids, x_axis, start_date, end_date)
    ...
    for i in range(len(x_axis)):
        ...
        if date_str in time_series:
            if unit == "Wh":
                ...
                consumption.append(round(time_series[date_str]["energy_consumed_in_Wh"] / 1000, 2) if time_series[date_str]["energy_consumed_in_Wh"] != 'null' else 'null')
                savings.append(round(time_series[date_str]["energy_saved_in_Wh"] / 1000, 2) if time_series[date_str]["energy_saved_in_Wh"] != 'null' else 'null')

            if unit == "MWh":
                consumption.append(round(time_series[date_str]["energy_consumed_in_Wh"] / 1000000, 2) if time_series[date_str]["energy_consumed_in_Wh"] != 'null' else 'null')
                savings.append(round(time_series[date_str]["energy_saved_in_Wh"] / 1000000, 2) if time_series[date_str]["energy_saved_in_Wh"] != 'null' else 'null')
        else:
            consumption.append('null')
            savings.append('null')
...
def get_data_of_individual_areas_of_a_year(db, area_ids, x_axis, start_date, end_date):
    consumption_data = {}
    consumption_data_combined = []
    filling_consumption_data_combined_first_time = True
    savings_data = {}
    ...
    for area_id in area_ids:
        ...
        if area:
            ...
            date_index = start_date
            while date_index < end_date:
                data = get_data_of_a_week(db, [area_id], [], date_index, date_index+timedelta(days=6))
                if date_index.strftime("%d") == "22":
                    date_index = date(date_index.year + (date_index.month == 12), (date_index.month % 12) + 1, 1)
                else:
                    date_index += timedelta(days=7)
            ...
            for i in range(len(x_axis)):
                ...
                if date_str in time_series:
                    ...
                    consumption_data[area_name].append(time_series[date_str]["energy_consumed_in_Wh"])
                    savings_data[area_name].append(time_series[date_str]["energy_saved_in_Wh"])

                    if filling_consumption_data_combined_first_time:
                        consumption_data_combined.append(time_series[date_str]["energy_consumed_in_Wh"])
                    elif time_series[date_str]["energy_consumed_in_Wh"] != 'null' and time_series[date_str]["energy_consumed_in_Wh"] != 'null':
                        consumption_data_combined[i] += time_series[date_str]["energy_consumed_in_Wh"]
                    elif time_series[date_str]["energy_consumed_in_Wh"] != 'null':
                        consumption_data_combined[i] = time_series[date_str]["energy_consumed_in_Wh"]

                    if filling_savings_data_combined_first_time:
                        savings_data_combined.append(time_series[date_str]["energy_saved_in_Wh"])
                    elif time_series[date_str]["energy_saved_in_Wh"] != 'null' and time_series[date_str]["energy_saved_in_Wh"] != 'null':
                        savings_data_combined[i] += time_series[date_str]["energy_saved_in_Wh"]
                    elif time_series[date_str]["energy_saved_in_Wh"] != 'null':
                        savings_data_combined[i] = time_series[date_str]["energy_saved_in_Wh"]
                else:
                    consumption_data[area_name].append('null')
                    savings_data[area_name].append('null')
